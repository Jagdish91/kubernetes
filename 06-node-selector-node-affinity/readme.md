...
Node Selector - Assigns pods to nodes based on simple key-value labels.
Node Affinity - Provides more flexibility, including soft preferences and multiple conditions.
...
nodeSelector:
  key: value
Pods are scheduled only on nodes matching the label
...
my-second-cluster-control-plane → This is the control-plane node responsible for managing the cluster.
my-second-cluster-worker → This is a worker node where application workloads can be scheduled.
my-second-cluster-worker2 → This is another worker node available for scheduling workloads.
...
📌 More on Set-Based Selectors: Day 10 - ReplicaSet
...
The scheduler finds all nodes that satisfy the requiredDuringSchedulingIgnoredDuringExecution rule.
It then evaluates each preferred rule (preferredDuringSchedulingIgnoredDuringExecution) and adds the node's weight to a sum.
The node with the highest final score is preferred, but other scheduling factors still apply.
...
At Scheduling Time: The scheduler assigns the pod to a node based on the labels present at that time.
After Scheduling: If the node’s labels change later, the pod remains on the same node unless manually deleted and recreated.
